...
class DATA_LOADER(object):
    ...
    def next_batch_rumi(self, batch_size):
        import math
        temp=torch.randperm(self.ntrain_class)
        
        neg=temp[:math.ceil(self.ntrain_class/2)]
        pos=temp[math.ceil(self.ntrain_class/2):]
        
        size_neg=math.ceil(batch_size/2)
        size_pos=math.floor(batch_size/2)
        
        batch_class_neg = torch.LongTensor(size_neg)
        batch_class_pos = torch.LongTensor(size_pos)
        for i in range(batch_class_neg):
        ...
        return batch_feature_p, batch_label_p, batch_att_p,batch_feature_n, batch_label_n, batch_att_n
