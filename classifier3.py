...
class CLASSIFIER:
    ...
    def __init__(self, opt, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=25,
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            acc, pred, output, all_acc, ensemble_acc, predicted_label_e, ensemble_output = self.val(
                self.test_unseen_feature, self.test_unseen_label, self.unseenclasses)

            if ensemble_acc > first_acc:
                first_acc = ensemble_acc
                first_all_pred = predicted_label_e
        ...
        for epoch in range(self.nepoch):
            ...
            correct_num = predicted_label_e.eq(mapped_test_label[keep_indices]).cpu().sum()
            ...
            if first_acc > acc_fc_hard:
                ...
                fc_hard_pred = predicted_label_e
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            acc_seen, pred_seen, output_seen = self.val_gzsl(self.test_seen_feature, self.test_seen_label, self.seenclasses,self.opt.gzsl_seen_output)
            acc_unseen, pred_unseen, output_unseen = self.val_gzsl(self.test_unseen_feature, self.test_unseen_label, self.unseenclasses,self.opt.gzsl_unseen_output)
        ...
        entropy_value = torch.from_numpy(np.asarray(list(map(entropy, first_all_output.data.detach().numpy()))))
        _, indices = torch.sort(-entropy_value)
        ...
        knn_hard_pred = self.train_Y[idx]
        knn_all_pred = torch.cat((first_easy_pred, knn_hard_pred), 0)
        knn_seen_pred, knn_seen_label, knn_unseen_pred, knn_unseen_label = self.split_pred(knn_all_pred,
                                                                                           all_easy_hard_label)
        ...
        print('1NN   Seen: {:.2f}%, Unseen: {:.2f}%, 1NN H: {:.2f}%'.format(acc_knn_seen * 100, acc_knn_unseen * 100,
                                                                            acc_knn_H * 100))
        ...
        for epoch in range(self.nepoch):
            ...
            #acc, pred, _ = self.val_gzslx(all_test_feature[keep_indices], first_hard_label, all_classes)
            acc, pred, _ = self.val_gzsl(all_test_feature[keep_indices], first_hard_label, all_classes,self.opt.allx(all_test_feature[keep_indices]))
        ...
        print('FC    Seen: {:.2f}%, Unseen: {:.2f}%, FC H: {:.2f}%'.format(acc_fc_seen * 100, acc_fc_unseen * 100,
                                                                           acc_fc_H * 100))
    ...
    def val_gzslx(self, test_X, test_label, target_classes):
        start = 0
        ntest = test_X.size()[0]
        predicted_label = torch.LongTensor(test_label.size())
        all_output = None
    ...
    def val_gzsl(self, test_X, test_label, target_classes,v2sout):
        ...
        ensemble_output = all_output + self.opt.ensemble_ratio * Variable(v2sout)
        _, predicted_labele = torch.max(ensemble_output.data, 1)
        predicted_labele=predicted_labele.cpu()
        ensemble_acc = self.compute_per_class_acc_gzsl(test_label, predicted_labele, target_classes)
        acc = self.compute_per_class_acc_gzsl(test_label, predicted_labele, target_classes)
        return ensemble_acc, predicted_labele, ensemble_output
